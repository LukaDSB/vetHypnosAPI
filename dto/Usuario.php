...
class Usuario {
    ...
    // ALTERADO: Construtor reflete a tabela
    ...
    // ALTERADO: fromArray reflete a tabela
    public static function fromArray(array $data): self {
        return new self(
            $data['id'] ?? null,
            $data['nome'],
            $data['email'],
            $data['senha'] ?? '', // Senha pode não vir do DB em alguns casos
            $data['crmv'] ?? null,
            $data['cpf'] ?? null,
    ...
    // ALTERADO: NUNCA retornar a senha (ou hash) para o frontend!
